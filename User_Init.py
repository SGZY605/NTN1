...
class user:
    def __init__(self, center_sat, cover_range, bs_list, sate_xyz,sate_lla, ontime, offtime, ue_xyz, ue_lat_lon_alt, index):
        ...
        self.init_user_position(self.center_sat, self.cover_range, self.bs_list, self.sate_lla, ue_xyz, ue_lat_lon_alt, index)#初始化用户的位置
        self.bs_if_serv = 0 # TODO:添加了bs_if_serv
        self.init_user_traffic(ontime, offtime, self.cbrrate)    # 初始化用户业务类信息


    def init_user_position(self, center, maxdistance, bs_list, sate_lla, ue_xyz, ue_lat_lon_alt, index):
        """
        初始化用户位置类信息:
        ...
        """
        self.ue_xyz, self.ue_lat_lon_alt = ue_xyz[index], ue_lat_lon_alt[index]
...
#以上为user类，都是类内函数
#以上为user类，都是类内函数
def initial_userAll(center_sat,cover_range, bs_list, user_num, sate_xyz, sate_lla, ontime, offtime, ue_xyz, ue_lat_lon_alt):
    """
    初始化用户，包含用户数量、用户的位置
    输入参数：
    ...
    user_list:用户列表
    """
    userlist = [user(center_sat,cover_range,bs_list, sate_xyz, sate_lla, ontime, offtime, ue_xyz, ue_lat_lon_alt, i) for i in range(user_num)]
    return userlist
